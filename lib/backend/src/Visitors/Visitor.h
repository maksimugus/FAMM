...
class LLVMIRGenerator : public tree::ParseTreeVisitor {
public:
    LLVMIRGenerator() : context(), builder(context), module("my_module", context) {}

    std::any visit(tree::ParseTree *node) override {
        if (auto program = dynamic_cast<FAMMParser::ProgramContext*>(node)) {
    ...
    llvm::Value* visitExpression(FAMMParser::ExpressionContext* expressionContext) {
        if (auto addSubCtx = dynamic_cast<FAMMParser::AddSubExpressionContext*>(expressionContext)) {
        ...
        } else if (auto mulDivCtx = dynamic_cast<FAMMParser::MulDivExpressionContext*>(expressionContext)) {
        ...
        } else if (auto constCtx = dynamic_cast<FAMMParser::ConstantExpressionContext*>(expressionContext)) {
